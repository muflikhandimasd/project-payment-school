...
class PembayaranController extends Controller
{
        ...
        if (!$pembayaran) {
            DB::transaction(function () use ($request, $petugas) {
                foreach ($request->bulan_bayar as $bulan) {
                    Pembayaran::create([
                        'kode_pembayaran' => 'SPPIDN' . Str::upper(Str::random(6)),
    ...
    public function printHistoryPembayaran($id)
    {
        ...
        return $pdf->stream("{$data['pembayaran']->siswa->kelas->nama_kelas}-{$data['pembayaran']->siswa->nama_siswa}-{$data['pembayaran']->bulan_bayar}-{$data['pembayaran']->tahun_bayar}.pdf");
    }
